...
function isScrollTarget(url: string) {
...
export default function NavBar() {
  ...
  function scrollToSection(event: React.MouseEvent<HTMLAnchorElement, MouseEvent>, section: string, isScroll: boolean) {
    if (isScroll) {
    ...
    <nav className={`${isMenuOpen ? 'flex opacity-100' : 'hidden opacity-0'} transition-opacity duration-500 fixed z-40 w-full h-full items-center justify-center`}>
      <div className="text-zinc-500 text-center">
        ...
        <ul className="pt-2.5 text-4xl md:text-5xl lg:text-7xl font-extrabold ">
          {menu.map((item) => {
            const scrollTarget = isScrollTarget(item.url)
            ...
            return (
              <li key={item.title} className="hover:text-white py-0.5 group">
                <a
                  ...
                  onClick={(event) => scrollToSection(event, item.url, scrollTarget)}
                >
